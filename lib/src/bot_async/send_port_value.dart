...
class SendPortValue<TInput, TOutput> extends FutureValue<TInput, TOutput> {
  ...
  void _sendValueResultCompleted(FutureValueResult<TOutput> value) {
    if(value.isException) {
      _completeException(value.error, value.stackTrace);
    } else {
  ...
  void _completeException(exception, stack) {
    final c = _completer;
    _completer = null;
    var iue = new IsolateUnhandledException('Error is SendPortValue', exception, stack);
    c.completeError(iue);
  }
